    ...
    @if(count($companies)>0)
        ...
        <div class="row">
				...
				<div class="col-md-2">
                                    ...
                                    <div class="row">
                                        <a class="text-center"href="/upload/{{$company->id}}">Upload</a>
                                    </div><br>
                                    @else
                                    @foreach($uploads as $upload)
                                    @if($company->id == $upload->company_id && $upload->deleted == "No")
                                    <div class="row">
						<a class="text-center"href="{{route('uploads.show', ['upload'=>$upload] )}}">View Uploads</a>
                                    </div><br>
                                    @endif
                                    @endforeach
                                    @endif
					...
					<div class="row">
						<a class="text-center"href="/trackings/{{$company->id}}">Tracking</a>
					</div>
                                        @else
                                        @foreach($trackings as $tracking)
                                        @if($company->id == $tracking->company_id && $tracking->deleted == "No")
                                        <div class="row">
						<a class="text-center"href="{{route('tracking.show', ['tracking'=>$tracking] )}}">View Tracking</a>
					</div>
                                        @endif
                                        @endforeach
                                        @endif
